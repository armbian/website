  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollReveal>
            <div className="text-center max-w-3xl mx-auto">
              <h1 className="text-fluid-3xl font-black tracking-tight leading-[0.95] mb-4">{t('title')}</h1>
              <p className="text-fluid-lg text-[rgb(var(--fg-2))] leading-relaxed">{t('subtitle')}</p>
            </div>
          </ScrollReveal>
        </div>
      </PageHero>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-24">
        {changelogs.length === 0 ? (
          <p className="text-center text-sm text-[rgb(var(--fg-3))] py-16">No releases published yet.</p>
        ) : (
          <div className="relative">
            <div className="absolute left-4 top-0 bottom-0 w-px bg-[rgb(var(--border)/0.3)]" />

            {changelogs.map((entry, i) => {
              const date = new Date(entry.releaseDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
              return (
                <ScrollReveal key={entry.id} delay={Math.min(i * 0.05, 0.3)} distance={20}>
                  <div className="relative pl-10 pb-10">
                    <div className={`absolute left-2.5 top-1 w-3 h-3 rounded-full border-2 ${entry.major ? 'bg-[rgb(var(--brand))] border-[rgb(var(--brand))]' : 'bg-[rgb(var(--bg))] border-[rgb(var(--border))]'}`} />
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex items-center gap-2">
                          <h2 className="text-base font-bold group-hover:text-[rgb(var(--brand))] transition-colors">{entry.version}</h2>
                          {entry.major && (
                        ...
                        <span className="text-[11px] text-[rgb(var(--fg-3))] font-mono shrink-0">{date}</span>
                      </div>

                      <p className="text-sm text-[rgb(var(--fg-2))] leading-relaxed mb-3">{entry.summary}</p>
