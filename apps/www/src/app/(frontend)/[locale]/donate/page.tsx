...
function DonateCard({ icon, iconBg, title, desc, cta, href }: {
  icon: ReactNode; iconBg: string; title: string; desc: string; cta: string; href: string;
}) {
  return (
    <div className="hw-card rounded-xl p-5 h-full flex flex-col relative overflow-hidden">
      <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-3 shrink-0" style={{ backgroundColor: iconBg }}>
        {icon}
      </div>
      ...
      <PageHero className="!pb-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <ScrollReveal>
            ...
            <p className="text-fluid-base text-[rgb(var(--fg-2))] leading-relaxed max-w-2xl mx-auto">{t('hero_subtitle')}</p>
          </ScrollReveal>
        </div>
      </PageHero>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 pt-4">
        <section className="mb-16">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {([
              { key: '1', icon: <Cpu size={18} strokeWidth={1.5} className="text-[rgb(var(--brand))]" /> },
              { key: '2', icon: <Server size={18} strokeWidth={1.5} className="text-[rgb(var(--brand))]" /> },
              { key: '3', icon: <Users size={18} strokeWidth={1.5} className="text-[rgb(var(--brand))]" /> },
              { key: '4', icon: <Wrench size={18} strokeWidth={1.5} className="text-[rgb(var(--brand))]" /> },
            ] as const).map((item, i) => (
              <ScrollReveal key={item.key} delay={i * 0.06} distance={25}>
                <div className="hw-card rounded-xl p-5 h-full relative overflow-hidden">
                  ...
                  <p className="text-xs text-[rgb(var(--fg-2))] leading-relaxed">{t(`why_${item.key}_desc`)}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
        ...
        <section className="mb-16">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <ScrollReveal delay={0} distance={20}>
              <DonateCard icon={<SiGithub size={22} className="text-white" />} iconBg="#24292f"
                title={t('method_github_title')} desc={t('method_github_desc')} cta={t('method_github_cta')} href={ARMBIAN_URLS.GITHUB_SPONSORS} />
            </ScrollReveal>
            <ScrollReveal delay={0.05} distance={20}>
              <DonateCard icon={<SiPaypal size={22} className="text-white" />} iconBg="#003087"
                title={t('method_paypal_title')} desc={t('method_paypal_desc')} cta={t('method_paypal_cta')} href={ARMBIAN_URLS.PAYPAL} />
            </ScrollReveal>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <ScrollReveal delay={0} distance={20}>
              <div className="hw-card rounded-xl p-6 h-full flex flex-col relative overflow-hidden">
                ...
                <p className="text-xs text-[rgb(var(--fg-2))] leading-relaxed mb-4 flex-1">{t('method_merch_desc')}</p>
                <a href={ARMBIAN_URLS.MERCH_EU} target="_blank" rel="noopener noreferrer"
            ...
            <ScrollReveal delay={0.05} distance={20}>
              <div className="hw-card rounded-xl p-6 h-full flex flex-col relative overflow-hidden">
                ...
                <p className="text-xs text-[rgb(var(--fg-2))] leading-relaxed mb-4 flex-1">{t('method_partnership_desc')}</p>
                <Link href="/partners" className="inline-flex items-center gap-2 text-sm font-semibold text-[rgb(var(--brand))] hover:underline">
            ...
            <div className="rounded-xl border border-[rgb(var(--border)/0.5)] bg-[rgb(var(--bg-el)/0.3)] divide-y divide-[rgb(var(--border)/0.3)]">
              {(['code', 'docs', 'test', 'community'] as const).map((key) => (
                <div key={key} className="flex items-start gap-4 px-5 py-4">
                  ...
                  <p className="text-sm text-[rgb(var(--fg-2))] leading-relaxed">{t(`other_${key}`)}</p>
                </div>
              ))}
            </div>
          </ScrollReveal>
